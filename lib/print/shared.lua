...
--- Resolve which resource asked for the log line: the one calling through
--- an export when there is one, this resource otherwise.
---@return string resource The resource credited with the line.
...
local function write(level, ...)
  ...
  _G.print(('%s[%s] [%s]%s %s'):format(level.color, resolveSource(), level.label, RESET, formatArgs(...)))
end
...
if not resolvedLevel then
  print.warn(("Unknown log level '%s' in convar '%s', using '%s'"):format(
    configuredLevel,
    CONVAR_LEVEL,
    DEFAULT_LEVEL
  ))
