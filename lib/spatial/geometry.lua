local SPHERE <const> = 'sphere'
local BOX <const> = 'box'
local POLY <const> = 'poly'
local BOX_CORNERS <const> = 4
...
local function containsSphere(shape, coords)
...
local function containsBox(shape, coords)
  if coords.z < shape.minZ or coords.z > shape.maxZ then
    return false
  end
...
---@param coords vector3 The coordinates to test.
---@return boolean inside Whether the point is inside the polygon.
local function containsPoly(shape, coords)
  if coords.z < shape.minZ or coords.z > shape.maxZ then
    return false
  end
...
local function containsZone(shape, coords)
...
local function createSphereShape(coords, radius)
...
local function createBoxShape(coords, size, heading)
...
local function createPolyShape(points, minZ, maxZ)
  local cx, cy = 0, 0
...
internal.SHAPE_SPHERE = SPHERE
internal.SHAPE_BOX = BOX
internal.SHAPE_POLY = POLY
internal.containsZone = containsZone
...
return {
  ...
  createPolyShape = createPolyShape,
}
