...
local DEFAULT_DISTANCE <const> = 10.0
...
local function getCameraForwardVector()
  local rot <const> = GetFinalRenderedCamRot(ROTATION_ORDER)
...
---@return table result The raycast result { hit, entityHit, endCoords, surfaceNormal, materialHash }.
local function raycastFromCoords(origin, destination, flags, ignoreEntity, ignoreFlags)
...
---@param ignoreFlags? number The ignore flags (default: 4).
---@return table result The raycast result { hit, entityHit, endCoords, surfaceNormal, materialHash }.
local function raycastFromCamera(distance, flags, ignoreEntity, ignoreFlags)
  local origin <const> = GetFinalRenderedCamCoord()
  local forward <const> = getCameraForwardVector()
  local dist <const> = distance or DEFAULT_DISTANCE
  ...
  return raycastFromCoords(origin, destination, flags, ignoreEntity, ignoreFlags)
end
...
---@param ignoreFlags? number The ignore flags (default: 4).
---@return table result The raycast result { hit, entityHit, endCoords, surfaceNormal, materialHash }.
local function raycastFromEntity(entity, distance, flags, ignoreEntity, ignoreFlags)
  ...
  return raycastFromCoords(origin, destination, flags, ignoreEntity or entity, ignoreFlags)
end

return {
  fromCoords = raycastFromCoords,
  fromCamera = raycastFromCamera,
  fromEntity = raycastFromEntity,
}
