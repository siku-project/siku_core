...
local function getClosestObject(coords, maxDistance, model)
  return findClosest(GetAllObjects(), coords, maxDistance, modelFilter(model))
end
...
local function getNearbyObjects(coords, maxDistance, model)
  return findNearby(GetAllObjects(), coords, maxDistance, modelFilter(model))
end
...
local function getClosestPed(coords, maxDistance, model, includePlayers)
  return findClosest(GetAllPeds(), coords, maxDistance, pedFilter(model, includePlayers))
end
...
local function getNearbyPeds(coords, maxDistance, model, includePlayers)
  return findNearby(GetAllPeds(), coords, maxDistance, pedFilter(model, includePlayers))
end

--- Get the closest vehicle to the given coordinates. The server has no local
--- player, so no vehicle is ever skipped as "the current one".
---@param coords vector3 The reference coordinates.
---@param maxDistance? number The maximum search distance (default: 2.0).
---@param model? string|number Optional model name or hash to filter by.
---@param includeCurrentVehicle? boolean Accepted for signature parity with the client, ignored on the server.
...
local function getClosestVehicle(coords, maxDistance, model, includeCurrentVehicle)
  return findClosest(GetAllVehicles(), coords, maxDistance, modelFilter(model))
end

--- Get all vehicles within a given distance, sorted by proximity. The server
--- has no local player, so no vehicle is ever skipped as "the current one".
---@param coords vector3 The reference coordinates.
---@param maxDistance? number The maximum search distance (default: 2.0).
---@param model? string|number Optional model name or hash to filter by.
---@param includeCurrentVehicle? boolean Accepted for signature parity with the client, ignored on the server.
...
local function getNearbyVehicles(coords, maxDistance, model, includeCurrentVehicle)
  return findNearby(GetAllVehicles(), coords, maxDistance, modelFilter(model))
end
