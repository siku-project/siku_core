...
local EVENTS <const> = {
  SERVER_REQUEST = 'siku:server:callbackRequest:%s',
  SERVER_RESPONSE = 'siku:server:callbackResponse:%s',
  CLIENT_REQUEST = 'siku:client:callbackRequest:%s',
  CLIENT_RESPONSE = 'siku:client:callbackResponse:%s',
}

local TIMEOUT <const> = math.max(tonumber(Siku.config.callback.timeout) or DEFAULT_TIMEOUT, MIN_TIMEOUT)
...
---@param sendRequest fun(target: number?, requestId: number, name: string, ...: any) Sends a request to the other side.
---@param sendResponse fun(target: number?, caller: string, requestId: number, ok: boolean, results: table) Answers a request, to the resource that asked.
...
local function createChannel(sendRequest, sendResponse, forwardTarget)
  ...
  ---@param caller string The resource that asked, which receives the answer.
  ...
  function channel.onRequest(target, caller, requestId, name, ...)
    local handler <const> = handlers[name]

    if not handler then
      sendResponse(target, caller, requestId, false, table.pack(("Callback '%s' is not registered"):format(tostring(name))))
      return
    end
    ...
    if not outcome[1] then
      ...
      sendResponse(target, caller, requestId, false, table.pack(reason))

      return
    end

    sendResponse(target, caller, requestId, true, table.pack(table.unpack(outcome, 2, outcome.n)))
  end
...
internal.createChannel = createChannel
