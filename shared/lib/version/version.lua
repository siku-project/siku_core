...
function Siku.CheckDependency(resource, minimumVersion)
  local caller <const> = GetInvokingResource() or Siku.name

  if type(resource) ~= 'string' or resource == '' then
    return buildResult(false, tostring(resource), 'malformed', tostring(minimumVersion), 'unknown',
  ...
  if not requested then
    return buildResult(false, resource, 'malformed', tostring(minimumVersion), 'unknown',
  ...
  if state == 'missing' or state == 'unknown' then
    return buildResult(false, resource, 'missing', minimumVersion, 'unknown',
      ("'%s' requires '%s' which is not installed"):format(caller, resource))
  end

  if state ~= 'started' then
    return buildResult(false, resource, 'stopped', minimumVersion, 'unknown',
  ...
  if not rawVersion then
    return buildResult(false, resource, 'unversioned', minimumVersion, 'unknown',
...
_SikuInternal.ParseVersion = parseVersion
_SikuInternal.CompareVersions = compareVersions
