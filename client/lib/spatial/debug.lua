...
local function updateTick()
  if next(debugZones) and not active then
    Siku.RegisterSpatialTick('zones:debug', debugTick)
    active = true
  elseif not next(debugZones) and active then
    Siku.UnregisterSpatialTick('zones:debug')
    active = false
  end
end
...
function Siku.EnableZoneDebug(id, shape, color)
...
---@param id number The zone ID.
function Siku.DisableZoneDebug(id)
  debugZones[id] = nil
  updateTick()
end
...
function Siku.IsZoneDebugActive(id)
